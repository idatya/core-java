...
public class FileParser {
	...
	private Optional<TypeInfo> getTypeInfo(Document document, String path, Statistics statistics) {
		...
		String fullType = body.select(".header h2").first().text();

		String packageName = body.select(".header > .subTitle").last().text();

		String declaration = body.select(".description > ul > li > pre").first().html();
